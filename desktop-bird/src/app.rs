...
impl AppState {
    ...
    /// Show a speech bubble above the bird for `duration` seconds.
    ///
    /// Replaces any bubble that is already showing.
    pub fn say(&mut self, text: impl Into<String>, duration: f32) {
        self.bubble = Some(BubbleState { text: text.into(), remaining: duration });
    }
