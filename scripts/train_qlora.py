...
import torch
# Accelerate's mixed-precision wrapper post-processes every model output by
# .float()-casting any bf16/fp16 tensors to fp32, *after* the model's forward
# has already returned the loss scalar. For Gemma 4 (vocab=262144, seq=256)
# that means a 204 MB fp32 upcast of the logits tensor that nothing
# downstream consumes — and it OOMs on 8 GB VRAM. Make it a no-op; the loss
# scalar is already fp32, and our chunked_causal_lm_loss does its own
# per-chunk upcast inside the autocast region.
from accelerate.utils import operations as _accel_ops  # noqa: E402
_accel_ops.convert_to_fp32 = lambda data: data
...
def load_quantized_model(model_id: str):
    ...
    # Replace the model's default loss_function (transformers.ForCausalLMLoss)
    # with a chunked variant. Default does `logits.float()` on the entire
    # tensor, which for vocab=262144 × seq=256 means a 268 MB fp32 allocation
    # in one shot — the lone OOM trigger on the 8 GB card after model+grads
    # are resident. Processing the upcast in token-chunks keeps peak at
    # ~17 MB instead of 268 MB, with identical numerics.
...
def chunked_causal_lm_loss(
    ...
    **kwargs,
    ...
    """Memory-efficient drop-in for transformers.ForCausalLMLoss.

    The default ForCausalLMLoss does `logits.float()` on the entire tensor
    and then F.cross_entropy on it, which materializes a (seq * vocab) fp32
    log-softmax tensor — for Gemma 4 (vocab=262144, seq=256) that's ~268 MB
    in one shot, unaffordable on 8 GB VRAM. We instead slice the logits
    into `chunk_size`-token bf16 strides and call F.cross_entropy per
    chunk; each internal log-softmax then peaks at chunk_size * vocab * 4
    bytes. Reduction semantics are identical to ForCausalLMLoss.
    """
    ...
    n_tokens = logits_flat.size(0)
    # Anchor to the autograd graph (1×1 slice × 0). Without this, batches
    # where every chunk's labels are ignore_index would leave total_loss as
    # a fresh `new_zeros` tensor with no grad_fn, breaking loss.backward().
    total_loss = logits_flat[0, 0].float() * 0.0
    total_count = 0
    for i in range(0, n_tokens, chunk_size):
        log_chunk = logits_flat[i:i + chunk_size]
        lab_chunk = labels_flat[i:i + chunk_size]
        valid = (lab_chunk != ignore_index).sum().item()
        if valid == 0:
            continue
...
class EmptyCacheCallback(TrainerCallback):
    """Release PyTorch's cached-but-unallocated GPU blocks after each step.

    On the 8 GB card, allocator fragmentation accumulates across training
    steps (paged_adamw_8bit warming up + variable per-batch seq lengths
    leaving misaligned free blocks). Without this, the backward of the
    lm_head OOMs by ~10-100 MB around step 5-10 even though most of the
    "in-use" memory is actually reusable.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--max-seq-length", type=int, default=160,
                    help="lm_head logits at vocab=262144 dominate VRAM. On 8 GB VRAM the worst-case grad_logits at seq=256 is ~134 MB and OOMs the backward; 160 caps it at ~80 MB which fits with our chunked CE + accelerate-fp32-patch. Response is preserved via tokenizer.truncation_side='left'; the few-shot block in the user prompt shrinks.")
    ...
    sft_config = SFTConfig(
        ...
        # Not actually using Liger — its registry has `gemma4_text` but not
        # the top-level `gemma4` multimodal wrapper we load, so the flag is
        # a no-op patcher. The reason we set it is that TRL's compute_loss
        # records a per-token entropy metric on the full logits tensor
        # (`entropy_from_logits` materializes exp(logps) * logps, vocab-
        # sized), gated by `elif not self.args.use_liger_kernel:`. Flipping
        # this true makes TRL skip that branch — saves ~64 MB of activations
        # at the moment of peak training-step memory.
